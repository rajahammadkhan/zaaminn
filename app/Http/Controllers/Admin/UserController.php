...
class UserController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = User::where('is_admin', ACTIVE)->where('name', '<>', 'Test');
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    $btn = '<div class="action__buttons">';
                    if ($data->status == ACTIVE) {
                        $btn .= '<a href="' . route('admin.customer_status_change', ['inactive', encrypt($data->id)]) . '" class="btn-action" title="Active"><i class="fas fa-toggle-on"></i></a>';
                    } else {
                        $btn .= '<a href="' . route('admin.customer_status_change', ['active', encrypt($data->id)]) . '" class="btn-action" title="Block"><i class="fas fa-toggle-off"></i></a>';
                    }
                    $btn .= '</div>';
                    return $btn;
                })
