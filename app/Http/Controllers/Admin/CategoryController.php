...
class CategoryController extends Controller
{
            ...
            return DataTables::of($data)
                ...
                ->editColumn('Category_Slug', function ($data) {
                 ...
                 ->editColumn('category_image', function ($data) {
                    $img = asset($data->category_image??'default_category.png');
                    return '<img src="' . $img . '" alt="' . $data->en_Category_Name . '" class="img-fluid" style="max-width: 50px;" ">';
                })
                ...
                ->rawColumns(['action', 'Category_Name', 'Category_Slug', 'Status', 'Description','category_image'])
                ->make(true);
        }
    ...
    {
        $image_path = 'default_category.png';
        if(isset($request->category_image) && $request->hasFile('category_image') ){
           $image_path = uploadFile($request->category_image, 'admin/categories', 'cat-');
