...
if (!function_exists('categoryWithBrands')) {
    function categoryWithBrands()
    {
        return Category::with('brands')->whereStatus(1)->get();
    }
}
...
if (!function_exists('sellerProductBrandCount')) {
    function sellerProductBrandCount($id)
    {
        $brand = Brand::with(['sellarProducts' => function ($query) {
            $query->where('is_admin_approval', 1);
        }])->find($id);

        if (!$brand) {
            return 0; // or handle the case when the brand with the given ID is not found
        }

        $count = $brand->sellarProducts->count();
